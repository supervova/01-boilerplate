...
--------------------------------------------------------------------------------
> НЕ ТРАТЬ ВРЕМЯ НА ЭТОТ ПРОЕКТ. Ты уже потратил кучу времени на современные
> инструменты, и отказался от них — см. ниже «Причины отказов от новых технологий».
> Забей! Оставь здесь Gulp. Он для статики. А Vite и пр. — для React.
> Это нормально. Ты не будешь всю жизнь лендосы собирать.
> ДЕЛАЙ MAGIC, ЧУВАК
--------------------------------------------------------------------------------
...
## Причины отказов от новых технологий

- npm-скрипты. В отличие от gulp, сложнее работа с путями. npm-скрипты [медленнее, чем gulp](https://deliciousbrains.com/npm-build-script/#verdict) и их набор получается сложнее, чем gulpfile или gulp-набор.
- Astro. Как и все остальное слишком сложен для простых задач SSG и будет отнимать больше времени, чем старый-добрый gulp.
- 11ty. Проблемы с путями при [нестандартной структуре проекта](https://github.com/mirisuzanne/teacup). Я еще не пробовал альтернативные структуры, например, из [моего ответвления](https://github.com/supervova/11ty-netlify/tree/main) одной из стартовых тем 11ty.
- Vite плохо совместим с шаблонизаторами. Однако я не попробовал [Vituum и его плагины](https://vituum.dev/plugins/pug.html). Впрочем, и там всё через жопу — pug-шаблоны инклюдятся в HTML.

## npm-скрипты

- Переделать с ChatGPT систему сборки на основе npm-скриптов
  - тестировать:
    - ✔️ sprite
    - js
    - pug
